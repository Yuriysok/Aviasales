    ...
    public class AeroflotAdapter : IAirlineAdapter
    {
        ...
        public AeroflotAdapter()
        {
            ResponseMapper = new MapperConfiguration(config =>
            {
                ...
                .ForMember(dest => dest.FlightId, src => src.MapFrom(x => x.Guid))
                ...
                .ForMember(dest => dest.Guid, src => src.MapFrom(x => x.FlightId))
        ...
        public record struct AeroflotFlight(string Guid, float PriceDollars, DateTime DepartureTime, DateTime ArrivalTime, int FlightsNumber);

        private record struct AeroflotBookingDto(string Guid, string PersonName, string PassportData);
    }
}
