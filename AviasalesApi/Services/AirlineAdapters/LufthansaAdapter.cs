...
namespace AviasalesApi.Services.AirlineAdapters
{
    public class LufthansaAdapter : IAirlineAdapter
    {
        ...
        public LufthansaAdapter()
        {
            ResponseMapper = new MapperConfiguration(config => {
                config.CreateMap<LufthansaFlight, Flight>()
                    ...
                    .ForMember(dest => dest.FlightId, src => src.MapFrom(x => x.FlightGuid))
                ...
                .ForMember(dest => dest.FlightGuid, src => src.MapFrom(x => x.FlightId))
        ...
        public record struct LufthansaFlight(string FlightGuid, float PriceUsd, DateTime TakeoffTime, DateTime LandingTime, int FlightNum);

        private record struct LufthansaBookingDto(string FlightGuid, string PassengerName, string PassportSerial);
    }
}
